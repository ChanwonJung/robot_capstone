...
## vgn_grasp_pkg와의 관계 — 드롭인 교체

두 패키지는 **동일한 출력 토픽과 JSON 스키마**를 사용한다. 런치파일만 교체하면 된다.
...
병렬 운용: 동시에 두 노드를 켜면 `/grasp_candidates`가 중복 발행되어 bt_pkg가 먼저 도착한 것을 소비한다. 의도적으로 경쟁시키거나 한쪽만 켜서 선택하면 된다.
