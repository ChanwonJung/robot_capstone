...
# torch/torchvision은 팀 환경에 이미 설치돼 있으면 생략
```
...
**공개 가중치 (ethz-asl/vgn 공식)**

1. [ethz-asl/vgn](https://github.com/ethz-asl/vgn) GitHub README → Google Drive 링크에서 `data.zip` 다운로드
2. 압축 해제 후 복사:

```bash
# 팀 레포 기준 (robot_capstone)
cp data/models/vgn_conv.pth ~/capstone_ws/robot_capstone/models/vgn_conv.pth
```
...
> **가중치는 gitignore 처리** (`models/*.pth`) — 팀원 각자 다운로드 필요.
